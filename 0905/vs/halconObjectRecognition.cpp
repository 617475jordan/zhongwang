...
void halconObjectRecognition::upDateNewData(char m_charFileName[100], int m_iCurrentRoi_X, int m_iCurrentRoi_Y, int m_iCurrentCols, int m_iCurrentRows, int m_iCurrentId)
{
	...
	setUpModel(m_charFileName, hv_vecHtRow.size() - 1,m_iCurrentId);
}


void halconObjectRecognition::setUpModel(char m_charFileName[100], int m_iCurrentFlag, int m_iCurrentId)
{
	// Local iconic variables
	HObject  ho_ModelImage, ho_ModelROI, ho_binThreshold;
	HObject  ho_ConnectedRegions, ho_SelectedRegions;
	HObject	 ho_ImageROI;
	HObject  ho_ShapeModelImage, ho_ShapeModelRegion, ho_ShapeModel;
	HObject  ho_RegionFillUp, ho_RegionDilation;
	...
	SelectShape(ho_ConnectedRegions, &ho_SelectedRegions, "area", "and", 50, 10000);
	...
	if (m_iCurrentId > 0)
	{
		sprintf(m_charFileName, m_strModelDst, m_iCurrentId);
