...
void onMouse(int event, int x, int y, int flags, void *param)
{
	...
	else if (event == CV_EVENT_LBUTTONUP)//左键松开，将在图像上划矩形 
	{
		...
		double m_dArea = m_icurrentRoi_Width* m_icurrentRoi_Height;
		double m_dMinArea = 50, m_dMaxArea = 2000;
		if (m_icurrentRoi_Width > 0 && m_icurrentRoi_Height > 0 /*&& m_dArea >= m_dMinArea&&m_dArea<=m_dMaxArea*/)
		{
			...
			sprintf(m_charDstImg, m_strDstImg, m_iCurrentId);
			imwrite(m_charDstImg, m_matCurrentImg);
			objectRecognition.upDateNewData(m_charDstImg, m_icurrentRoi_X, m_icurrentRoi_Y, m_icurrentRoi_Width, m_icurrentRoi_Height, m_iCurrentId);
			m_iCurrentId++;
