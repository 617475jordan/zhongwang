...
Mat drawing::drawingContourMomentAndPointCentroid(Mat m_matCommonImg, Mat m_matMainImg, vector<vector<vector<Point>>> m_vecPointFeatures, vector<string> m_vecStrMediaName)
{
	...
	{
		m_iKeeping = -1;
		return drawingImage(m_matMainImg, m_vecPointFeatures, m_vecStrMediaName);
	}
	...
	for (unsigned int i = 0; i < m_vecContours.size(); i++)
	{
		...
		double m_iCurrentArea = contourArea(m_vecContours[i]);
		if (m_iCurrentArea>80 /*&& m_iCurrentArea < m_dAreaThreshold * 5*/)
		{
			m_vecFilterContours.push_back(m_vecContours[i]);
...
Mat drawing::drawingImage(Mat m_matMainImg, vector<vector<vector<Point>>> m_vecPointFeatures, vector<string> m_vecStrMediaName)
{
	...
	for (unsigned int i = 0; i < m_vecRectangle.size(); i++)
	{
		...
		if (matchShape.findMainRectContours(m_rect) == true)
		{
			...
			m_vecMaxValueImg = matchShape.computeContoursSimilarity(m_vecPointFeatures);
			//cout << "m_vecMaxValueImg:" << m_vecMaxValueImg.size() << endl;
