...
void MutilObjetctRecognitionBasedOnQt::on_screenShootButton_clicked()
{
	if (m_bCap == true)
	{
		...
		ho_ModelImage = matToHalcon.IplImageToHImage(m_matCurrentImg);
...
void MutilObjetctRecognitionBasedOnQt::mouseReleaseEvent(QMouseEvent *e)
{
	...
	{	
		...
		{
			//sprintf(m_charDstImgName, m_strDstImg, m_iCurrentImgId);
			objectRecognition.upDateNewData(ho_ModelImage, m_icurrentRoi_X, m_icurrentRoi_Y, m_icurrentRoi_Width, m_icurrentRoi_Height, m_iCurrentImgId);
...
void MutilObjetctRecognitionBasedOnQt::on_mixComboBox_clicked()
{
	...
	{
		ui.showComboBox->clear();
		ui.showComboBox->setCurrentText("Original");
	}
...
void MutilObjetctRecognitionBasedOnQt::imagetransform()
{
	...
	/*ui.showComboBox->setCurrentText("RightToLeft");*/

	if (m_bimagetransform == true)
	{
		ui.showComboBox->setCurrentText("RightToLeft");
	}
...
{
	ui.showComboBox->clear();
	if (m_bimageEnhance == true)
	{
		ui.showComboBox->setCurrentText(m_qOrignialimageEnhance);
	}
	ui.showComboBox->addItem("Orignial");
...
cv::Mat MutilObjetctRecognitionBasedOnQt::imageEnhancement(Mat m_mat)
{
	...
	{
		m_qOrignialimageEnhance = ui.showComboBox->currentText();
		m_bimageEnhance = true;
		String m_strCurrentImg = m_qOrignialimageEnhance.toStdString();
		if (m_strCurrentImg == "Orignial")
		{
			m_strCurrentImg.clear();
			return m_mat;
		}
		else if (m_strCurrentImg == "Histogram")
		{
		...
		else if (m_strCurrentImg == "Laplace")
		{
			Mat m_matLaplace;
		...
		else if (m_strCurrentImg == "Auto_White_Balance")
		{
...
cv::Mat MutilObjetctRecognitionBasedOnQt::imageTransform(Mat m_mat)
{
	...
	{
		m_qorignialimagetransform = ui.showComboBox->currentText();
		m_bimagetransform = true;
		String m_strCurrent = m_qorignialimagetransform.toStdString();
		m_orignialimagetransform = m_strCurrent;
	...
	{
		if (m_orignialimagetransform == "Orignial")
		{
			return m_mat;
		}
		else if (m_orignialimagetransform == "RightToLeft")
		{
			remapImage remapImage;
			m_mat = remapImage.imageRemap(m_mat);
		}
		else  if (m_orignialimagetransform == "AboveToBelow")
		{
			flip(m_mat, m_mat, -1);
		}
		else if (m_orignialimagetransform == "Other")
		{
			remapImage remapImage;
			m_mat = remapImage.imageRemap(m_mat);
			flip(m_mat, m_mat, -1);
