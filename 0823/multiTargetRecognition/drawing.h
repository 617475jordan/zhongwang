...
public:
	drawing();
	~drawing();
	Mat                       drawingContourMomentAndPointCentroid(Mat m_matCommonImg, Mat m_matMainImg, vector<vector<vector<Point>>> m_vecPointFeatures, vector<string> m_vecStrMediaName);
	Mat                       drawingImage(Mat m_matMainImg , vector<vector<vector<Point>>> m_vecPointFeatures, vector<string> m_vecStrMediaName);
private:
