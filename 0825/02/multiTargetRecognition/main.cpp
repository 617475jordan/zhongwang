	...
	/*	Mat kernel = (Mat_<float>(3, 3) << 0, -1, 0, 0, 5, 0, 0, -1, 0);
		...
		if (m_icount >= 1)
		{
			...
			m_matImg = drawing.drawingContourMomentAndPointCentroid(m_matForegroundImg, m_matImg,
				m_vecPointFeatures, m_vecStrMediaName, m_vecVecCommonKeyPoint, m_vecMat);
		}
...
int   initialize()
{
	...
	for (int j = 1; j <= m_iPictureNum; j++)
	{
		...
		cvtColor(m_matTmpImg, m_matGrayImg, CV_RGB2GRAY);
		cvtColor(m_matGrayImg, m_matTmpImg, CV_GRAY2RGB);
		if (matchShape.findAllCommonRectContours(m_matTmpImg).size() != 0)
		{
			...
			m_vecVecCommonKeyPoint.push_back(matchShape.commonKeyPoint());
			m_vecMat.push_back(m_matGrayImg);
		}
	}
	m_matTmpImg.release();
	matchShape.clear();
