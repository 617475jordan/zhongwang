...
Mat drawing::drawingContourMomentAndPointCentroid(Mat m_matCommonImg, Mat m_matMainImg, vector<vector<vector<Point>>> m_vecPointFeatures, 
	vector<string> m_vecStrMediaName, vector<vector<KeyPoint>> m_vecVecCommonKeyPoint, vector<Mat> m_vecMat)
{
	...
	{
		m_iKeeping = -1;
		return drawingImage(m_matMainImg, m_vecPointFeatures, m_vecStrMediaName, m_vecVecCommonKeyPoint, m_vecMat);
	}
	...
	vector<vector<KeyPoint>> m_vecVecCommonKeyPoint, vector<Mat> m_vecMat)
{
	...
	for (unsigned int i = 0; i < m_vecRectangle.size(); i++)
	{
		...
		if (matchShape.findMainRectContours(m_rect) == true)
		{
			...
			//int t = clock();
			m_vecMaxValueImg = matchShape.computeContoursSimilarity(m_vecPointFeatures, m_vecStrMediaName, m_vecVecCommonKeyPoint, m_vecMat);
			//cout << "ktime:" << clock() - t << "ms" << endl;
